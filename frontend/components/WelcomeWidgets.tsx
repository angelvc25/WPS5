      ...
      `}</style>
        ...
        <View style={styles.widgetGrid}>
          ...
          <View style={styles.widgetRow}>
            {activeOrder.slice(0, 5).map((id: string, colIdx: number) => {
              const slotIndex = colIdx;
              const isBeingMoved = Boolean(isMoveMode && movingWidgetId === id);
              return isWidgetVisible(id) && (
                <View key={id} style={{ position: 'relative', overflow: 'visible' }}>
                  {renderWidget(id, slotIndex, isBeingMoved)}
                </View>
              );
            })}
          </View>
          ...
          {/* Row 2 */}
          <View style={[styles.widgetRow, { marginTop: 12 }]}>
            {activeOrder.slice(5, 10).map((id: string, colIdx: number) => {
              const slotIndex = 5 + colIdx;
              const isBeingMoved = Boolean(isMoveMode && movingWidgetId === id);
              return isWidgetVisible(id) && (
                <View key={id} style={{ position: 'relative', overflow: 'visible' }}>
                  {renderWidget(id, slotIndex, isBeingMoved)}
